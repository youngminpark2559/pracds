# Original source of this code is from Anisotropic's Spooky NLP and Topic Modelling tutorial
# https://www.kaggle.com/arthurtok/spooky-nlp-and-topic-modelling-tutorial/code
# I add my additional comment and result of each code execution onto his code

# Topic modelling is process,
# in which we try uncover abstract themes or "topics",
# based on underlying documents and words in corpus. 

# I will introduce two standard topic modelling techniques
# Latent Dirichlet Allocation (LDA) and Non-negative Matrix Factorization (NMF). 
# I will also introduce Tokenization, Stemming, Vectorization of raw text.

# Outline of this notebook:

# 1. Exploratory Data Analysis (EDA) and Wordclouds:
# Analyzing data by generating simple statistics such word frequencies over different authors 
# as well as plotting some wordclouds.
# 1. NLP with NLTK :
# Introducing basic text processing methods 
# such as tokenizations, stop word removal, stemming and vectorizing text via term frequencies (TF) 
# as well as inverse document frequencies (TF-IDF)
# 1. Topic Modelling with LDA and NNMF:


# @
import base64
import numpy as np
import pandas as pd
import plotly.offline as py
# py.init_notebook_mode(connected=False)
import plotly.graph_objs as go
import plotly.tools as tls
from collections import Counter
from scipy.misc import imread
from sklearn.feature_extraction.text import TfidfVectorizer, CountVectorizer
from sklearn.decomposition import NMF, LatentDirichletAllocation
from matplotlib import pyplot as plt
# %matplotlib inline


# You load training data with Pandas
# c train: train data in dataframe
train = pd.read_csv("../input/train.csv")

# @
# 1. 3 Authors and their works EDA
# First step, let us look at first three rows in data and who are authors
# print("train.head()",train.head())
# c train.head():
#         id                                               text author
# 0  id26305  This process, however, afforded me no means of...    EAP
# 1  id17569  It never once occurred to me that the fumbling...    HPL
# 2  id11008  In his left hand was a gold snuff box, from wh...    EAP
# 3  id27763  How lovely is spring As we looked from Windsor...    MWS
# 4  id12958  Finding nothing else, not even gold, the Super...    HPL

# We have three author's initials
# EAP - Edgar Allen Poe
# HPL - HP Lovecraft
# MWS - Mary Shelley

# Next, let us look at how large training data is:
# print("train.shape",train.shape)
# c train.shape (19579, 3)

 # @
# Summary statistics of training set
# we can visualize basic statistics in data, 
# like distribution of entries for each author. 
# I will invoke Plot.ly library and plot them with bar plots. 

# c z: dictionary for initial and fullname of 3 authors
z = {"EAP": "Edgar Allen Poe", "MWS": "Mary Shelley", "HPL": "HP Lovecraft"}

# c data: data (x:3 authors, y:counts, marker, text) for plot in list
data=[go.Bar(
            x = train.author.map(z).unique(),
            y = train.author.value_counts().values,
            marker= dict(colorscale="Jet",
                         color = train.author.value_counts().values
                        ),
            text="Text entries attributed to Author"
    )]

# # c layout: layout object for title text 
layout = go.Layout(
    title='Target variable distribution'
)

# c fig: figure object which stores data list and layout object​
fig=go.Figure(data=data, layout=layout)
# py.iplot(fig, filename='basic-bar')
# py.plot(fig, filename='basic-bar')

# c all_words: 
all_words = train['text'].str.split(expand=True).unstack().value_counts()
data = [go.Bar(
            x = all_words.index.values[2:50],
            y = all_words.values[2:50],
            marker= dict(colorscale='Jet',
                         color = all_words.values[2:100]
                        ),
            text='Word counts'
    )]
layout = go.Layout(
    title='Top 50 (Uncleaned) Word frequencies in the training dataset'
)
fig = go.Figure(data=data, layout=layout)
# py.plot(fig, filename='basic-bar')
# py.iplot(fig, filename='basic-bar')




# Can you notice anything odd about words which appear in this word frequency plot?
# Do these words actually tell us much about themes and concepts
# which Mary Shelley wants to portray to reader in her stories?

# These words are commonly occuring words which you could find anywhere. 
# Not just in this example by 3 authors but also in newspapers, every other english text. 
# Therefore we must find some way to preprocess our dataset first 
# to strip out all these commonly occurring words which do not bring much to table.

# @
# WordClouds to visualize each author's work
# One handy visualization tool for data scientist when it comes to NLP is plotting "Word Cloud". 

# Here instead of dealing with actual book or text, 
# our words can be taken from column "text"

# @
# Store text of each author in Python list

# We create 3 lists which store texts of 3 authors.

# c eap: all text of eap, list
eap = train[train.author=="EAP"]["text"].values
# c hpl: all text of hpl, list
hpl = train[train.author=="HPL"]["text"].values
# c mws: all text of mws, list
mws = train[train.author=="MWS"]["text"].values

from wordcloud import WordCloud, STOPWORDS

# Generating normal wordcloud is boring
# So I'd like to introduce to you technique of importing pictures
# And you will use outline of that picture as mask for wordclouds. 
# Therefore pictures that I have chosen are ones I feel most representative for their authors:

# 1) Raven for Edgar Allen Poe 2.) Octopus Cthulu-thingy for HP Lovecraft and 3.) Frankenstein for Mary Shelly

# The way I am loading in pictures on Kaggle is feature hack 
# although readers whic are familiar to my work know this trick. 
# I derive Base64 encoding of whatever images I want to use 
# And then I use that particular encoding and re-convert picture back on notebook. 

# Cell below contains Base64 encoding of three images I will use 
# But I have hidden them so that I do not pollute this notebook with long streteches of text
# You can unhide them if you want to see encoding.

# c eap_64: picture of eap hashed by base64
eap_64 = b''

# c hpl_64: picture of hpl hashed by base64
hpl_64 = b''

# c mws_64: picture of mws hashed by base64
mws_64 = b''



# To decode image, 
# I import "codecs" module 
# And I save it as new image here in this notebook. 
# Once image is saved, I can load it as mask as follows:

import codecs
# You generate Mask for EAP
f1 = open("eap.png", "wb")
f1.write(codecs.decode(eap_64,'base64'))
f1.close()
img1 = imread("eap.png")
# img = img.resize((980,1080))
# c hcmask: mask of eap, list
hcmask = img1
# print("hcmask",hcmask)

# You generate Mask for MWS
f2 = open("mws.png", "wb")
f2.write(codecs.decode(mws_64,'base64'))
f2.close()
img2 = imread("mws.png")
# c hcmask2: mask of mws, list
hcmask2 = img2

# You generate Mask for HPL
f3 = open("hpl.png", "wb")
# You decode hashed string into picture,
# and you write decoded picture into hpl.png file
f3.write(codecs.decode(hpl_64,'base64'))
f3.close()
img3 = imread("hpl.png")
# c hcmask3: mask of hpl, list
hcmask3 = img3;

# @
# Finally plotting word clouds via following few lines (unhide to see the code):
plt.figure(figsize=(20,18))
# The wordcloud of the raven for Edgar Allen Poe
# Note: meaning of 211
plt.subplot(211)

wc = WordCloud(
    background_color="black", 
    max_words=10000, 
    mask=hcmask, 
    stopwords=STOPWORDS, 
    max_font_size= 40)
wc.generate(" ".join(eap))
plt.title("Edgar Allen Poe (The Raven)")
plt.imshow(wc.recolor( colormap= 'PuBu' , random_state=17), alpha=0.9)
plt.axis('off')
# plt.show()

plt.figure(figsize=(20,18))
wc = WordCloud(
    background_color="black", 
    max_words=10000, 
    mask=hcmask2, 
    stopwords=STOPWORDS, 
    max_font_size= 40)
wc.generate(" ".join(mws))

# print("np.array(mws).shape",np.array(mws).shape)
# np.array(mws).shape (6044,)

# print("mws",mws)
# mws ['How lovely is spring As we looked from Windsor Terrace on the sixteen fertile counties spread beneath, speckled by happy cottages and wealthier towns, all looked as in former years, heart cheering and fair.'
#  'A youth passed in solitude, my best years spent under your gentle and feminine fosterage, has so refined the groundwork of my character that I cannot overcome an intense distaste to the usual brutality exercised on board ship: I have never believed it to be necessary, and when I heard of a mariner equally noted for his kindliness of heart and the respect and obedience paid to him by his crew, I felt myself peculiarly fortunate in being able to secure his services.'
#  'I confess that neither the structure of languages, nor the code of governments, nor the politics of various states possessed attractions for me.'
#  ...
#  'Once my fancy was soothed with dreams of virtue, of fame, and of enjoyment.'
#  'Nay, you may have met with another whom you may love; and considering yourself as bound in honour to Elizabeth, this struggle may occasion the poignant misery which you appear to feel." "My dear father, reassure yourself.'
#  'Stress of weather drove us up the Adriatic Gulph; and, our vessel being hardly fitted to weather a storm, we took refuge in the port of Ancona.']


# list=["ab cd.","ef gh "]
# print("j_l",j_l)
# c j_l: ab cd. ef gh

# list=["ab cd. "," ef gh "]
# j_l=" ".join(list)
# print("j_l",j_l)
# j_l ab cd.   ef gh 

# print('" ".join(mws)'," ".join(mws))
# c " ".join(mws):
# ...
# I instantly wrote to Geneva; nearly two months have elapsed since the departure of my letter. But I was at once startled and enchanted by my sudden extension of vision, when the curtain, which had been drawn before the intellectual world, was withdrawn, and I saw the universe, not only as it presented itself to my outward senses, but as it had appeared to the wisest among men. My life had hitherto been remarkably secluded and domestic, and this had given me invincible repugnance to new countenances.
# ...
plt.title("Mary Shelley (Frankenstein's Monster)", fontsize= 18)
plt.imshow(wc.recolor( colormap= 'viridis' , random_state=17), alpha=0.9)
plt.axis('off')
# plt.show()

# Wordcloud of Cthulhu/squidy thing for HP Lovecraft
# Note: how shape (16,13) is determined?
plt.figure(figsize=(16,13))
wc = WordCloud(
    background_color="black", 
    max_words=10000,
    mask=hcmask3, 
    stopwords=STOPWORDS, 
    max_font_size= 40)
wc.generate(" ".join(hpl))
plt.title("HP Lovecraft (Cthulhu-Squidy)", fontsize=20)
# plt.imshow(wc.recolor( colormap= 'Pastel1_r' , random_state=17), alpha=0.98)
plt.imshow(wc.recolor( colormap= 'Pastel2' , random_state=17), alpha=0.98)
plt.axis('off')
# plt.show()

# @
# You have 3 separate word clouds. 
# You can see from word clouds some of choice words that were favoured by different authors.

# You can see HP lovecraft favours words like "dream", "time", "strange", "past", "ancient",
# which seem to resonate with themes that author was famous for, 

# On other hand, one can see that Mary Shelley's words revolve around primal instincts 
# And themes of morality which range from positive to negative ends of spectrum, 
# such as "friend", "fear", "hope", "spirit" etc.

# However, as you can see from word clouds, 
# there are still handful of words that seem to be quite out of place. 
# Words such as "us", "go", "he" which seem to appear commonly every where in text


# @
# 2. Natural Language Processing

# In all NLP tasks, you will come across 
# (topic modelling, word clustering, document-text classification etc), 
# few pre-processing steps to convert input raw text into form 
# which is readable by your model and machine.

# You certainly can't expect to feed Random Forest model paragraph of words
# And you can't expect RF to predict which author that paragraph came from. 

# Behind scenes, text pre-processing can be boiled down to a few steps:

# Tokenization
# Stopwords
# Stemming
# Vectorization

# NLTK: To make our NLP endeavours more convenient

# @
import nltk

# @
# 2a. Tokenization
# You store first text element as string
first_text = train.text.values[0]
# print("first_text",first_text)
# c first_text:: This process, however, afforded me no means of ascertaining the dimensions of my dungeon; as I might make its circuit, and return to the point whence I set out, without being aware of the fact; so perfectly uniform seemed the wall.
# print("="*90)
# print(first_text.split(" "))
# c first_text.split(" "):: ['This', 'process,', 'however,', 'afforded', 'me', 'no', 'means', 'of', 'ascertaining', 'the', 'dimensions', 'of', 'my', 'dungeon;', 'as', 'I', 'might', 'make', 'its', 'circuit,', 'and', 'return', 'to', 'the', 'point', 'whence', 'I', 'set', 'out,', 'without', 'being', 'aware', 'of', 'the', 'fact;', 'so', 'perfectly', 'uniform', 'seemed', 'the', 'wall.']


# As you can see from first attempt at tokenization, 
# segregation of sentence into its individual elements (or terms) is not entirely accurate. 
# Punctuation mark (comma) has also been included 
# Ideally we would like comma and word to be in two different and separate elements of list. 

# Trying to do this with list operations will be complex.
# NLTK library comes into play. 
# There is "word_tokenize( )" (TreebankWord tokenizer) 
# which strips out singular words 
# as well as punctuations into separate elements automatically as follows:

# c first_text_list: tokenized first text by nltk tokenizer
first_text_list = nltk.word_tokenize(first_text)
# print(first_text_list)
# ['This', 'process', ',', 'however', ',', 'afforded', 'me', 'no', 'means', 'of', 'ascertaining', 'the', 'dimensions', 'of', 'my', 'dungeon', ';', 'as', 'I', 'might', 'make', 'its', 'circuit', ',', 'and', 'return', 'to', 'the', 'point', 'whence', 'I', 'set', 'out', ',', 'without', 'being', 'aware', 'of', 'the', 'fact', ';', 'so', 'perfectly', 'uniform', 'seemed', 'the', 'wall', '.']

# @
# 2b. Stopword Removal
# c stopwords: stopwords list
stopwords = nltk.corpus.stopwords.words('english')

# print("len(stopwords)",len(stopwords))
# len(stopwords) 179

# print("stopwords",stopwords)
# stopwords ['i', 'me', 'my', 'myself', 'we', 'our', 'ours', 'ourselves', 'you', "you're", "you've", "you'll", "you'd", 'your', 'yours', 'yourself', 'yourselves', 'he', 'him', 'his', 'himself', 'she', "she's", 'her', 'hers', 'herself', 'it', "it's", 'its', 'itself', 'they', 'them', 'their', 'theirs', 'themselves', 'what', 'which', 'who', 'whom', 'this', 'that', "that'll", 'these', 'those', 'am', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'having', 'do', 'does', 'did', 'doing', 'a', 'an', 'the', 'and', 'but', 'if', 'or', 'because', 'as', 'until', 'while', 'of', 'at', 'by', 'for', 'with', 'about', 'against', 'between', 'into', 'through', 'during', 'before', 'after', 'above', 'below', 'to', 'from', 'up', 'down', 'in', 'out', 'on', 'off', 'over', 'under', 'again', 'further', 'then', 'once', 'here', 'there', 'when', 'where', 'why', 'how', 'all', 'any', 'both', 'each', 'few', 'more', 'most', 'other', 'some', 'such', 'no', 'nor', 'not', 'only', 'own', 'same', 'so', 'than', 'too', 'very', 's', 't', 'can', 'will', 'just', 'don', "don't", 'should', "should've", 'now', 'd', 'll', 'm', 'o', 're', 've', 'y', 'ain', 'aren', "aren't", 'couldn', "couldn't", 'didn', "didn't", 'doesn', "doesn't", 'hadn', "hadn't", 'hasn', "hasn't", 'haven', "haven't", 'isn', "isn't", 'ma', 'mightn', "mightn't", 'mustn', "mustn't", 'needn', "needn't", 'shan', "shan't", 'shouldn', "shouldn't", 'wasn', "wasn't", 'weren', "weren't", 'won', "won't", 'wouldn', "wouldn't"]

# To filter out stop words from our tokenized list of words, 
# we can use list comprehension as follows:
# c first_text_list_cleaned: first text which is removed stopwords
first_text_list_cleaned = [word for word in first_text_list if word.lower() not in stopwords]
# print(first_text_list_cleaned)
# ['process', ',', 'however', ',', 'afforded', 'means', 'ascertaining', 'dimensions', 'dungeon', ';', 'might', 'make', 'circuit', ',', 'return', 'point', 'whence', 'set', ',', 'without', 'aware', 'fact', ';', 'perfectly', 'uniform', 'seemed', 'wall', '.']
# print("="*90)
# print("Length of original list before stopwords removal: {0} words\n"
#       "Length of list after stopwords removal: {1} words"
#       .format(len(first_text_list), len(first_text_list_cleaned)))
# Length of original list before stopwords removal: 48 words
# Length of list after stopwords removal: 28 words


# As you can see, our list with stopwords removed is now substantially shorter than before where commonly occuring words such as "I", "me", "to" and "the" have been removed.



# @
# 2c. Stemming and Lemmatization
# c stemmer: nltk PorterStemmer object
stemmer = nltk.stem.PorterStemmer()

# We can use stemmer to see if it can reduce our these test words 
# ("running", "runs", "run") into their single stemmed word. 
# Conveniently we can test stemmer on fly as follows:
print("The stemmed form of running is: {}".format(stemmer.stem("running")))
print("The stemmed form of runs is: {}".format(stemmer.stem("runs")))
print("The stemmed form of run is: {}".format(stemmer.stem("run")))
# The stemmed form of running is: run
# The stemmed form of runs is: run
# The stemmed form of run is: run

# Stemmer has reduced given words above into base form 
# And stemmer will be in helping us reduce size of our dataset of words 
# when we come to learning and classification tasks.

# However there is one flaw with stemming 
# And flaw is fact that process involves quite crude heuristic in chopping off ends of words 
# in hope of reducing particular word into human recognizable base form. 
# Therefore this process does not take into account vocabulary or word forms 
# when collapsing words as this example will illustrate:

# print("The stemmed form of leaves is: {}".format(stemmer.stem("leaves")))
# nltk.download("wordnet")

# @
# Lemmatization to rescue
# Here we can utilize NLTK to initialize lemmatizer (WordNet variant) 
# And we can inspect how it collapses words as follows:
from nltk.stem import WordNetLemmatizer
# c lemm: WordNetLemmatizer object
lemm = WordNetLemmatizer()
# print("The lemmatized form of leaves is: {}".format(lemm.lemmatize("leaves")))
# The lemmatized form of leaves is: leaf

# Lemmatizer is working and collapsing words into form that makes much more lexical sense.

# @
# 2d. Vectorizing Raw Text

# There are many different purposes for analyzing raw text
# In some cases you would like to compare similarity of one body of text to another (Clustering techniques/Distance measurements), text classification (the purpose of this competition) as well as uncovering topics that comprise body of text (the aim of this notebook). 

# We will convert our text into numbers.
# We will use Bag-of-Words

# @
# The Bag of Words approach
# With 2 sentences, "I love to eat Burgers", "I love to eat Fries",
# you should create dictionary of 6 words
# [I, love, to, eat, Burgers, Fries]. 

# Vectorizing text via Bag of Words approach, 
# we get 6 vectors, one for each word. 
# You will utilize Scikit-learn library to implement vectorizer
# Vectorizer will generate vector of word counts (term frequencies) via CountVectorizer()
# Defining our sentence
sentence = ["I love to eat Burgers",
            "I love to eat Fries"]

# c vectorizer: CountVectorizer object

# Note: v
# According to jupyter notebook explanation, min_df means minimum document frequency.
# I'm understanding min_df=0 means from 2 sentences you exclude all words which don't show up 0 times.
# So, min_df=0 means you don't exclude any words.
# Is my understanding right?
# So, I tested with min_df=2
# Following code print("The features are:\n {}".format(vectorizer.get_feature_names())) creates output like
# The features are:
#  ['eat', 'love', 'to']
# Why doesn't it include "I" even if "I" shows up 2 times from 2 sentences?
vectorizer = CountVectorizer(min_df=0)
# vectorizer = CountVectorizer(min_df=2)
# c sentence_transform: transformed sentences via CountVectorizer object
sentence_transform = vectorizer.fit_transform(sentence)
# print("sentence_transform",sentence_transform)
# c sentence_transform   
#   (0, 0)	1
#   (0, 1)	1
#   (0, 4)	1
#   (0, 3)	1
#   (1, 2)	1
#   (1, 1)	1
#   (1, 4)	1
#   (1, 3)	1

# Note: v
# I don't understand above tuple's meaning.
# I'm not able to match concept what I understand about "bag of word" to these tuples.
# What would each number mean?
# I guess first number of tuple means each sentence like first sentence, second sentence.

# I add my guess on tuple question.
# Second index number of tuple looks like indicating "word"
# First sentence noted by 0 in tuple doesn't have "2" (maybe "Fries")
# And number right next to tuple looks like number of frequency of word.
# For example "I" shows up one time in first sentence.
# But I still don't get how each word is mapped to particular number.

# @
# Fitting vectorizer to dataset

# "min_df" refers to minimum document frequency 
# And vectorizer will simply drop all words that occur less than that value set 
# (either integer or in fraction form). 

# Finally we apply fit_transform() is actually comprised of two steps. 
# First step is fit() where vectorizer is mapped to dataset that you provide. 
# Once this is done, actual vectorizing operation is performed 
# via transform() where raw text is turned into its vector form

print("The features are:\n {}".format(vectorizer.get_feature_names()))
# The features are:
#  ['burgers', 'eat', 'fries', 'love', 'to']

print("\nThe vectorized array looks like:\n {}".format(sentence_transform.toarray()))
# The vectorized array looks like:
#  [[1 1 0 1 1]
#   [0 1 1 1 1]]

# Note:
# I also don't get understanding on 
# print("\nThe vectorized array looks like:\n {}".format(sentence_transform.toarray()))
# # The vectorized array looks like:
# #  [[1 1 0 1 1]
# #   [0 1 1 1 1]]

# with 
# sentence = ["I love to eat Burgers",
#             "I love to eat Fries"]

# I'm just guessing 5 elements in one list indicate 5 words of each sentence.
# But what about number?

# @
# Sparse matrix vector ouptuts

# From output of vectorized text, 
# we can see that features consist of words in corpus of text that we fed into vectorizer 
# (here corpus being two sentences we defined earlier). 
# Simply call get_feature_names attribute from vectorizer to inspect it.

# With regards to transformed text, 
# one would be tempted to inspect values by simplying calling it. 
# However when you try to call it you really just get message 
# which states "sparse matrix of type class 'numpy.int64' 
# with 8 stored elements in Compressed Sparse Row format". 
# Therefore this means that vectorizer returns transformed raw text as sparse matrix 
# Thinking about this, it makes sense that our returned matrices contain quite high degree of sparsity

# @
# 3. Topic modelling via LDA and NMF
# I will implement two different topic modelling techniques

# 1. Latent Dirichlet Allocation:
# Probabilistic, generative model which uncovers topics latent to dataset 
# by assigning weights to words in corpus, 
# where each topic will assign different probability weights to each word.

# 1. Non-negative Matrix Factorization :
# Approximation method that takes input matrix and approximates factorization of this matrix 
# into two other matrices, with caveat that values in matrix be non-negative.


# You define helper function to print top words
def print_top_words(model, feature_names, n_top_words):
    """
    This method prints top words
    Args:
        model:
        feature_names:
        n_top_words:
    """
    for index, topic in enumerate(model.components_):
        message = "\nTopic #{}:".format(index)
        message += " ".join([feature_names[i] for i in topic.argsort()[:-n_top_words - 1 :-1]])
        print(message)
        print("="*70)

# 3a. Putting all preprocessing steps together
# When you vectorize raw text with CountVectorizer, 
# dual stages of tokenizing and stopwords filtering are 
# automatically included as high-level component. 

# Here unlike NLTK tokenizer, Sklearn's tokenizer discards all single character terms 
# like ('a', 'w' etc) and also lower cases all terms by default. 

# Unfortunately, there is no built-in lemmatizer in vectorizer
# So we are left with couple of options. 
# 1. Implementing it separately everytime before feeding data for vectorizing 
# 1. Extending sklearn implementation to include this functionality. 

# Luckily for us, we have latter option 
# where we can extend CountVectorizer class by overwriting build_analyzer()

# @
# Extending CountVectorizer class with lemmatizer

# c lemm: WordNetLemmatizer object
lemm = WordNetLemmatizer()
class LemmaCountVectorizer(CountVectorizer):
    def build_analyzer(self):
        """
        member variable:
            analyzer:
            super class CountVectorizer's analyzer
        Returns:

        """
        analyzer = super(LemmaCountVectorizer, self).build_analyzer()
        return lambda doc: (lemm.lemmatize(w) for w in analyzer(doc))

# We have inherited and subclassed original Sklearn's CountVectorizer class 
# And we've overwritten build_analyzer() by implementing lemmatizer for each list in raw text matrix.


# Storing entire training text in list
text = list(train.text.values)
# print("np.array(text).shape",np.array(text).shape)
# c np.array(text).shape (19579,)

# Calling our overwritten Count vectorizer
# c tf_vectorizer: LemmaCountVectorizer object
tf_vectorizer = LemmaCountVectorizer(max_df=0.95, 
                                     min_df=2,
                                     stop_words='english',
                                     decode_error='ignore')
tf = tf_vectorizer.fit_transform(text)
# print("tf",tf)
# tf   (0, 13324)	1
#   (0, 12815)	1
#   (0, 8820)	1
#   (0, 4557)	1
#   (0, 942)	1
#   (0, 10820)	1
#   (0, 9115)	1
#   (0, 10220)	1
#   (0, 1999)	1
#   (0, 7432)	1
#   (0, 3802)	1
#   (0, 3383)	1
#   (0, 738)	1
#   (0, 7579)	1
#   (0, 276)	1
#   (0, 9400)	1
#   (1, 7783)	1
#   (1, 7662)	1
#   (1, 5148)	1
#   (1, 8330)	1
#   (2, 10567)	1
#   (2, 10761)	1
#   (2, 9203)	1
#   (2, 5478)	1
#   (2, 336)	1
#   :	:
#   (19574, 4612)	1
#   (19574, 7280)	1
#   (19575, 2068)	1
#   (19575, 7135)	1
#   (19575, 11336)	1
#   (19576, 4672)	1
#   (19576, 8500)	1
#   (19576, 5078)	1
#   (19576, 7425)	1
#   (19576, 6079)	1
#   (19576, 4573)	1
#   (19576, 10589)	1
#   (19577, 2585)	1
#   (19577, 11650)	1
#   (19577, 6703)	1
#   (19577, 8132)	1
#   (19577, 9841)	1
#   (19577, 7162)	1
#   (19578, 10850)	1
#   (19578, 10939)	1
#   (19578, 421)	1
#   (19578, 5384)	1
#   (19578, 7759)	1
#   (19578, 6942)	1
#   (19578, 2053)	1




# Revisiting our Term frequencies

# Having implemented our lemmatized count vectorizer, 
# let us revist plots for term frquencies of top 50 words (by frequency). 
# As you can see from plot, all our prior preprocessing efforts have not gone to waste. 
# With removal of stopwords, remaining words seem much more meaningful 
# where you can see that all stopwords in earlier term frequency plot

feature_names = tf_vectorizer.get_feature_names()
# print("feature_names",feature_names)
# feature_names ['aback', 'abandon', ...,'zone', 'ædile', 'æronaut']

count_vec = np.asarray(tf.sum(axis=0)).ravel()
# print("count_vec",count_vec)
# [ 2 11 29 ...  3  2  2]
zipped = list(zip(feature_names, count_vec))
# print("zipped",zipped)
# c zipped [('aback', 2), ('abandon', 11), ('abandoned', 29), ('abandoning', 3), ('abandonment', 5), ('abaout', 24),...]
x, y = (list(x) for x in zip(*sorted(zipped, key=lambda x: x[1], reverse=True)))
# print("type((list(x) for x in zip(*sorted(zipped, key=lambda x: x[1], reverse=True))))",type((list(x) for x in zip(*sorted(zipped, key=lambda x: x[1], reverse=True)))))
# <class 'generator'>

# print("x",x)
# x ['time', 'man', 'day', 'thing', 'eye', 'said', 'did', 'old', 'like', 'life', 'night', 'thought', ...]

# print("y",y)
# y [869, 779, 743, 725, 707,...]

# Now I want to extract out on the top 15 and bottom 15 words
# c Y: concatenated term frequency data from top 15 and bottom 15
Y = np.concatenate([y[0:15], y[-16:-1]])
# c X: concatenated word data from top 15 and bottom 15
X = np.concatenate([x[0:15], x[-16:-1]])

# You plot Plot.ly plot for Top 50 Word frequencies after Preprocessing
data = [go.Bar(
            x = x[0:50],
            y = y[0:50],
            marker= dict(colorscale='Jet',
                         color = y[0:50]),
            text='Word counts')]

layout = go.Layout(
    title='Top 50 Word frequencies after Preprocessing'
)

fig = go.Figure(data=data, layout=layout)
# py.iplot(fig, filename='basic-bar')
# py.plot(fig, filename='basic-bar')

# Plotting the Plot.ly plot for Bottom 100 Word frequencies after Preprocessing
data = [go.Bar(
            x = x[-100:],
            y = y[-100:],
            marker= dict(colorscale='Portland',
                         color = y[-100:]
                        ),
            text='Word counts'
    )]

layout = go.Layout(
    title='Bottom 100 Word frequencies after Preprocessing'
)

fig = go.Figure(data=data, layout=layout)
# py.iplot(fig, filename='basic-bar')
# py.plot(fig, filename='basic-bar')


# 3b. Latent Dirichlet Allocation
# We will talk about topic modelling and implementation of unsupervised learning algorithms. 
# There are different implements of LDA algorithm
# In this notebook, I will be using Sklearn's implementation. 
# Another very well-known LDA implementation is Radim Rehurek's gensim

# Corpus - Document - Word : Topic Generation

# In LDA, modelling process revolves around 3 things: 
# 1. text corpus
# 1. its collection of documents,
# 1. D and words W in documents. 

# Therefore algorithm attempts to uncover K topics from this corpus via following way (illustrated by diagram)


# There is model each topic k via Dirichlet prior distribution given by $$$\beta_{k}$$$:
# $$$\beta_{k} \sim \text{Dirichlet}(\eta), k=1,...,K$$$

# There is model each document d by another Dirichlet distribution parameterized by $$$\alpha$$$ :
# $$$\beta_{k} \sim \text{Dirichlet}(\eta), k=1,...,K$$$

# Subsequently for document d, 
# we generate topic via multinomial distribution 
# which we then backtrack and use to generate correspondings words related to that topic 
# via another multinomial distribution:
# $$$z_{di}\sim\text{Multinomial}(\theta_{d}) $$$
# $$$w_{ij}\sim\text{Multinomial}(\beta_{z_{di^{*}}}) $$$
# (Image source: http://scikit-learn.org/stable/modules/decomposition.html#latentdirichletallocation)

# The LDA algorithm first models documents via mixture model of topics. 
# From these topics, words are then assigned weights based on probability distribution of these topics. 
# It is this probabilistic assignment over words that allow user of LDA to say 
# how likely particular word falls into topic. 
# Subsequently from collection of words assigned to particular topic, 
# are we thus able to gain insight as to what that topic may actually represent from lexical point of view.

# From standard LDA model, 
# there are few key parameters that we have to keep in mind 
# And you should consider programmatically tuning before we invoke model:
# n_components: number of topics which you specify to model
# $$$\alpha$$$ parameter: This is dirichlet parameter which can be linked to document topic prior
# $$$\beta$$$ parameter: This is dirichlet parameter which can be linked to topic word prior

# To invoke algorithm, we simply create LDA instance through Sklearn's LatentDirichletAllocation function. 
# various parameters would ideally have been obtained through some sort of validation scheme. 
# In this instance, optimal value of n_components (or topic number) was found 
# by conducting KMeans + Latent Semantic Analysis Scheme 
# whereby number of Kmeans clusters and number of LSA dimensions were 
# iterated through and best silhouette mean score.

# c lda: LatentDirichletAllocation object
lda = LatentDirichletAllocation(n_components=11, 
                                max_iter=5,
                                learning_method = 'online',
                                learning_offset = 50.,
                                random_state = 0)
lda.fit(tf)
# c lda.fit(tf)::
# LatentDirichletAllocation(batch_size=128, doc_topic_prior=None,
                        #   evaluate_every=-1, learning_decay=0.7,
                        #   learning_method='online', learning_offset=50.0,
                        #   max_doc_update_iter=100, max_iter=5, mean_change_tol=0.001,
                        #   n_components=11, n_jobs=1, n_topics=None, perp_tol=0.1,
                        #   random_state=0, topic_word_prior=None,
                        #   total_samples=1000000.0, verbose=0)


# Topics which are generated by LDA
# We will utilize helper function print_top_words() to return top 10 words
# which are attributed to each of LDA generated topics. 
# To select number of topics, this is handled through parameter n_components in function.

# c n_top_words: number of top words for topics
n_top_words = 40
print("\nTopics in LDA model: ")
tf_feature_names = tf_vectorizer.get_feature_names()
print_top_words(lda, tf_feature_names, n_top_words)

first_topic = lda.components_[0]
second_topic = lda.components_[1]
third_topic = lda.components_[2]
fourth_topic = lda.components_[3]

# print("first_topic.shape",first_topic.shape)

# @
# Word Cloud visualizations of the topics

# print("first_topic.argsort()",first_topic.argsort())
# c first_topic.argsort() [13624  6431  1924 ...  4557  8147  7579]

# print("first_topic.argsort()",np.array(first_topic.argsort()).shape)
# first_topic.argsort() (13781,)

# Note: 2018-05-14 12-49-45
# I can't understand this indexing argsort()[:-50 - 1 :-1]
# even with comparing results of them
# print("first_topic.argsort()",first_topic.argsort())
# c first_topic.argsort() [13624  6431  1924 ...  4557  8147  7579]
# print("first_topic.argsort()",np.array(first_topic.argsort()).shape)
# first_topic.argsort() (13781,)
# print("first_topic.argsort()[:-50 - 1 :-1]",first_topic.argsort()[:-50 - 1 :-1])
# first_topic.argsort()[:-50 - 1 :-1] [ 7579  8147  4557 13750 10220  5476  5981  7282 13610  2650  3362  5477
#  13527  4785 10820  9198  9943 11673  8806   139 11224  7025  5727  6846
#   4684  7941  4369  4334  1733  5901 13687  6939 13092  9833    73  4959
#   9107  8087  2356  6639  9528   942  3622 11039  5244   421  5947  8416
#  12263  6996]
# print("first_topic.argsort()[:-50 - 1 :-1]",np.array(first_topic.argsort()[:-50 - 1 :-1]).shape)
# first_topic.argsort()[:-50 - 1 :-1] (50,)
# Does anyone have opinion?

first_topic_words = [tf_feature_names[i] for i in first_topic.argsort()[:-50 - 1 :-1]]
second_topic_words = [tf_feature_names[i] for i in second_topic.argsort()[:-50 - 1 :-1]]
third_topic_words = [tf_feature_names[i] for i in third_topic.argsort()[:-50 - 1 :-1]]
fourth_topic_words = [tf_feature_names[i] for i in fourth_topic.argsort()[:-50 - 1 :-1]]

# @
# Word cloud of First Topic
# Generating the wordcloud with the values under the category dataframe
firstcloud = WordCloud(
    stopwords=STOPWORDS,
    background_color='black',
    width=2500,
    height=1800).generate(" ".join(first_topic_words))
plt.imshow(firstcloud)
plt.axis('off')
# plt.show()

# Generating the wordcloud with the values under the category dataframe
cloud = WordCloud(
    stopwords=STOPWORDS,
    background_color='black',
    width=2500,
    height=1800).generate(" ".join(second_topic_words))
plt.imshow(cloud)
plt.axis('off')
# plt.show()

# Generating the wordcloud with the values under the category dataframe
cloud = WordCloud(
    stopwords=STOPWORDS,
    background_color='black',
    width=2500,
    height=1800).generate(" ".join(third_topic_words))
plt.imshow(cloud)
plt.axis('off')
# plt.show()

# Generating the wordcloud with the values under the category dataframe
cloud = WordCloud(
    stopwords=STOPWORDS,
    background_color='black',
    width=2500,
    height=1800).generate(" ".join(fourth_topic_words))
plt.imshow(cloud)
plt.axis('off')
# plt.show()
